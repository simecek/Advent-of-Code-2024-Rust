...
fn main() -> io::Result<()> {
    ...
    for sequence in &mut second_section {
        ...
        loop {
            ...
            let mut valid = true;
            let mut problematic_value = None;

            for i_j in sequence.iter() {
                ...
                if let Some(values) = map.get(i_j) {
                    for &value in values {
                        ...
                        if !visited.contains_key(&value) {
                            println!("Validation failed for value {} in sequence {:?}", value, sequence); // Debug failure
                            valid = false;
                            problematic_value = Some((value, *i_j));
                            break;
                        }
                    }
                }
            ...
            if valid {
                ...
                println!("Middle element: {}", sequence[middle_index]);
                if changed {
                    final_output += sequence[middle_index]; // Add middle element to final_output
                }
                break;
            } else if let Some((value, i_j)) = problematic_value {
                if let Some(pos) = sequence.iter().position(|&x| x == value) {
                    changed = true;
                    sequence.remove(pos);
                    if let Some(index) = sequence.iter().position(|&x| x == i_j) {
                        sequence.insert(index, value);
                    } else {
                        sequence.push(value);
                    }
                }
                println!("Fixed sequence: {:?}", sequence);
            } else {
                println!("0");
                break;
            }
